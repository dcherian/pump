...
class facetgrid:
    ...
    def map_col(
        ...
        if data is not None:
            for ax, loc in zip(self.col_axes[col_name], self.row_locs):
                ...
                self.handles[col_name].append(hdl)
    ...
    def map_row(
        ...
        if data is not None:
            for ax, loc in zip(self.row_axes[row_name], self.col_locs):
                ...
                if self.handles["first"] is None:
                    self.handles["first"] = hdl
                self.handles[row_name].append(hdl)
    ...
    def set_xlabels(self, label=None, row=None, **kwargs):
        ...
        if row is None:
            ...
            ax = self.row_axes[row]

        if label is None:
            label = self.x
        if isinstance(label, str):
            label = [label]
        for ax, text in zip_longest(ax, label, fillvalue=label[0]):
    ...
    def set_ylabels(self, label=None, **kwargs):
        ...
        if isinstance(label, str):
            label = [label]
        for ax, text in zip_longest(self.axes[:, 0], label, fillvalue=label[0]):
