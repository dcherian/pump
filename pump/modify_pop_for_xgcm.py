...
def _add_pop_dims_to_dataset(ds):
    ...
    ds_new["nlon_t"] = xr.Variable(
        ("nlon_t"), np.arange(len(ds.nlon)) + 0.5, {"axis": "X"}
    )
    ds_new["nlat_t"] = xr.Variable(
        ("nlat_t"), np.arange(len(ds.nlat)) + 0.5, {"axis": "Y"}
    )
