...
def estimate_euc_depth_terms(ds):
    ...
    ds['ueuc'] = ds.u.interp(depth=ds.eucmax, longitude=ds.longitude)
    ds['du'] = ds.us - ds.ueuc
    ds.du.attrs['long_name'] = '$\Delta$u'

    if 'dens' in ds:
        ds['b'] = ds.dens * -9.81/1025 + 9.81
        ds['bs'] = ds.b.isel(depth=0)
        ds['beuc'] = ds.b.interp(depth=ds.eucmax, longitude=ds.longitude)
        ds['db'] = ds.bs - ds.beuc
...
tao_ctd = (pump.obs.read_tao()
           ...
           .mean('time')
           .compute())
tao_ctd['dens'] = pump.mdjwf.dens(tao_ctd.salt, tao_ctd.temp, tao_ctd.depth)
...
tao = xr.merge([tao_adcp[['us', 'ueuc', 'du']],
                tao_ctd[['bs', 'beuc', 'db']]])
#+END_SRC

*** Simple Ri calculation

Another way to do this is to think of Ri=0.5 or something far east (e..g. 195W). Given crude estimates of longitudinal changes in Δb, EUCmax depth etc. can we explain the drop in Ri to 0.25 by 170W?
