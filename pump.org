...
def estimate_euc_depth_terms(ds):
    ...
    if 'dens' in ds:
        ds['dens_euc'] = ds.dens.interp(depth=ds.eucmax, longitude=ds.longitude, method='linear')
...
hires = xr.load_dataset('pump/glade/small-annual-mean.nc')
hires['eucmax'] = (pump.calc.get_euc_max(hires.u)
                   .rolling(longitude=100, center=True, min_periods=1)
                   .mean())
...
#+RESULTS:
:RESULTS:
| <Figure | size | 2000x2500 | with | 7 | Axes> | (Ri : <matplotlib.axes._subplots.AxesSubplot at 0x2ab666c2c0f0> h : <matplotlib.axes._subplots.AxesSubplot at 0x2ab66b2124a8> du : <matplotlib.axes._subplots.AxesSubplot at 0x2ab6696091d0> db : <matplotlib.axes._subplots.AxesSubplot at 0x2ab66691f278> u : <matplotlib.axes._subplots.AxesSubplot at 0x2ab6659b4a90> b : <matplotlib.axes._subplots.AxesSubplot at 0x2ab666947908> contrib : <matplotlib.axes._subplots.AxesSubplot at 0x2ab66bfe7c18>) |
...
#+RESULTS:
:RESULTS:
: /gpfs/u/home/dcherian/python/xarray/xarray/core/nanops.py:160: RuntimeWarning: Mean of empty slice
:   return np.nanmean(a, axis=axis, dtype=dtype)
: <matplotlib.legend.Legend at 0x2af7398d24a8>
[[file:images/Ri-diagnosis-gcm1.png]]
:END:

**** Small et  al 2014, CESM1 hires

- need to smooth EUC max
#+BEGIN_SRC jupyter-python
hires.eucmax.plot()
...
#+RESULTS:
:RESULTS:
| <matplotlib.lines.Line2D | at | 0x2adda920ef98> |
[[file:./.ob-jupyter/e2fda7d9f0654bc4d203b11d1ae38c7bed9a974a.png]]
:END:


Ri looks OK actually *but* Δb seems to be as import as Δu between 200 and 170.
...
#+begin_export latex
\Ri = \frac{\bar{w} h Q}{\left((-ghη_x + Δb/2 h h_x) + τ_w \right)²}
#+end_export
...
#+BEGIN_SRC jupyter-python
dirname = '/glade/p/cesm/community/ASD-HIGH-RES-CESM1/hybrid_v5_rel04_BC5_ne120_t12_pop62/ocn/proc/tseries/monthly'

kwargs = dict(data_vars='minimal', coords='minimal', compat='override',
...
cesm = (xr.merge([u, T, S], compat='override')
        .rename({'UVEL': 'u',
                 "TEMP": "temp",
                 "SALT": "salt",
        }))
...
cesm = cesm.rename({'nlon': 'longitude'})
cesm['z_t'] /= -100
cesm['z_t'].attrs['units'] = 'm'
cesm = cesm.sel(z_t=slice(0, -600))

annual = cesm.mean('time').compute()
annual = annual.sel(longitude=slice(None, 268))
annual['longitude'] -= 360
annual = annual.rename({'z_t': 'depth'})

annual['u'] /= 100
annual['u'].attrs['units'] = 'm/s'

annual['dens'] = pump.mdjwf.dens(annual.salt, annual.temp, annual.depth)
annual.attrs['name'] = 'hires CESM1'
annual.to_netcdf('pump/glade/small-annual-mean.nc')
#+END_SRC
